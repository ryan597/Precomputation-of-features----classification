...
if __name__ == '__main__':
	...
	for i, label in enumerate(train_labels):
		...
		for image_path1 in file_names:
			try:
				img1 = image.load_img(image_path1, target_size=image_size)
				img1 = image.img_to_array(img1)[None]

				img2 = image.load_img('data/flow/train/'+label+'/'+image_path1[-16:-4]+'.png', target_size=image_size)
				img2 = image.img_to_array(img2)

				x = preprocess_input(img1)
				y = preprocess_input(img2)[None]

				IRfeature = model.predict(x)
				Flofeature = model.predict(y)
				feature = np.concatenate((IRfeature, Flofeature), 1)
		...
		file_names = glob.glob(cur_path + "/output/*.jpg")
		file_names = sorted(file_names)
		for image_path1 in file_names:
			try:
				...
				image_path2 = random.choice(glob.glob('data/flow/train/'+label+'/output/*'+str(int(image_path1.split('_')[2][:-4]))+'*'))
