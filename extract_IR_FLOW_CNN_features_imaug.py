...
if __name__ == '__main__':
	...
	for i, label in enumerate(train_labels):
		...
		for image_path1 in file_names:
			try:
				image_path2 = random.choice(glob.glob(cur_path+'/output/*'+str((int(image_path1.split('_')[2][:-4])+10))+'*'))
				img1 = image.load_img(image_path1, target_size=image_size)
				img1 = image.img_to_array(img1)[:, :, 0][None]
				img2 = image.load_img(image_path2, target_size=image_size)
				img2 = image.img_to_array(img2)[:, :, 0][None]
				image_path3 = random.choice(glob.glob('data/flow/train/'+label+'/output/*'+str(int(image_path1.split('_')[2][:-4]))+'*'))
				img3 = image.load_img(image_path3, target_size=image_size, color_mode='grayscale')
				img3 = image.img_to_array(img3)[:,:,0][None]
				x = np.concatenate((img1, img2, img3), 0)
				x = np.transpose(x, (2, 1, 0))[None]
				x = preprocess_input(x)
